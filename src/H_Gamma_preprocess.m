...
binlist_location = 'C:\Users\douwe\OneDrive\Documents\Github\EEG_to_ERP_pipeline_stats_R\testing\scripts\';
...
%         EEG = eeg_checkset( EEG );
%         %finding the bad channels for the participant using the old bad channels
%         for i=1:length(participant_badchan)
%             if strcmp(subject_list{s}, participant_badchan(i,1))
%                 to_delete=participant_badchan(i,2);
%             end
%         end
%         bad_channels=convertStringsToChars(split(to_delete));
...
%         
        ...
        EEG  = pop_binlister( EEG , 'BDF', [binlist_location '\binlist_assr.txt'], 'IndexEL',  1, 'SendEL2', 'EEG', 'Voutput', 'EEG' );
        EEG = pop_epochbin( EEG , epoch_time,  baseline_time); %epoch size and baseline size
        %deleting bad epochs
        EEG= pop_artmwppth( EEG , 'Channel', 1:EEG.nbchan, 'Flag',  1, 'Threshold',  200, 'Twindow', epoch_time, 'Windowsize',  200, 'Windowstep',  200 );% to flag bad epochs
